...
export default class Carousel extends Component {
  ...
  render() {
    let state = this.state;
    let {renderDots,dots, infinite, children} = this.props;
    let pages = [];
    ...
    return (
      <View onLayout={this.onLayout}>
        ...
        {renderDots?renderDots(this.state.selectedIndex,count):(dots ? this.renderDots(this.state.selectedIndex) : this.renderDots2(this.state.selectedIndex))}
      </View>
    );
  }
}
